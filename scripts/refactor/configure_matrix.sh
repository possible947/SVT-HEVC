...
# Default configure (no behavior change expectation)
cmake -S . -B "${BUILD_ROOT}/default" -DCMAKE_BUILD_TYPE=Release \
  > "${LOG_DIR}/configure-default.log" 2>&1

# Static + LTO configure scaffold check
cmake -S . -B "${BUILD_ROOT}/static-lto" -DCMAKE_BUILD_TYPE=Release \
